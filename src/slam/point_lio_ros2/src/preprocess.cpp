  ...
  deque<double> & time_lidar, const int required_frame_num, int scan_count)
{
  ...
  sort(pl_surf.points.begin(), pl_surf.points.end(), time_list_cut_frame); //根据点云的时间戳前后排列
...
void Preprocess::avia_pc2_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg){
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  for (int i = 0; i < pl_orig.points.size(); i++){
    ...
    double dist = pl_orig.points[i].x * pl_orig.points[i].x +
