...
string root_dir = "/home/ld/code/ros2/colcon_2_point_lio_pb/src/Point-LIO-grid_map_ros2/";
// string root_dir = ament_index_cpp::get_package_share_directory("point_lio");
...
PointCloudXYZI::Ptr mapping_effected(new PointCloudXYZI());
...
void MapIncremental()
{
  ...
  for (size_t i = 0; i < cur_pts; ++i) {
    ...
    if (!Nearest_Points[i].empty()) { //邻域不为空，降采样
  ...
  // std::cout << "points_to_add size: " << feats_down_world->size() << "\n";
...
void publish_effected_mapping(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr & pubLaserCloudFullRes){
  int size = feats_down_world->points.size();
  PointCloudXYZI::Ptr laserCloudWorld(new PointCloudXYZI(size, 1));
  for (int i{0}; i < size; i++) {
    laserCloudWorld->points[i].x = feats_down_world->points[i].x;
  ...
  *mapping_effected += *laserCloudWorld;
  std::cout << "mapping_effected_size: " << mapping_effected->size() << "\n";
  sensor_msgs::msg::PointCloud2 laserCloudmsg;
  pcl::toROSMsg(*mapping_effected, laserCloudmsg);
  laserCloudmsg.header.stamp = get_ros_time(lidar_end_time);
  laserCloudmsg.header.frame_id = "camera_init";
  pubLaserCloudFullRes->publish(laserCloudmsg);
}
...
int main(int argc, char ** argv){  
  ...
  downSizeFilterMap.setLeafSize(filter_size_map_min, filter_size_map_min, filter_size_map_min);  //未使用
  ...
  // auto pub_laser_cloud_full_res = nh->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_registered_world", 1000);
  auto pub_laser_cloud_full_res_body = nh->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_registered_body", 1000);
  // auto pub_laser_cloud_effect = nh->create_publisher<sensor_msgs::msg::PointCloud2>("cloud_effected_map", 1000);
  // auto pub_laser_cloud_map = nh->create_publisher<sensor_msgs::msg::PointCloud2>("Laser_map", 1000);
  auto pub_odom_aft_mapped = nh->create_publisher<nav_msgs::msg::Odometry>("point_lio_odom", 1000);
  // auto pub_path = nh->create_publisher<nav_msgs::msg::Path>("point_lio_path", 1000);
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      {
        time_seq = time_compressing<int>(feats_down_body);  //每一个点的时间戳
      ...
      // publish_effected_mapping(pub_laser_cloud_effect);
