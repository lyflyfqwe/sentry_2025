...
struct BoxPointType{
    std::array<float, 3> vertex_min;  
    std::array<float, 3> vertex_max;  

    BoxPointType(): vertex_min{0}, vertex_max{0} {}

    BoxPointType(const BoxPointType& other) = default;

    // BoxPointType(const BoxPointType& other){
    //     for(int i{0}; i < 3; i++){
    //         vertex_min[i] = other.vertex_min[i];
    //         vertex_max[i] = other.vertex_max[i];
    //     }
    // }

    template<typename T>
    BoxPointType(const std::vector<T>& vec){
        if(vec.size() < 6) throw std::invalid_argument("Vector must have at least 6 elements.");  
        for(int i{0}; i < 3; i++){
            vertex_min[i] = vec[2*i];
            vertex_max[i] = vec[2*i+1];
        }
    }

    template<typename T> BoxPointType&
