...
void RMMDecision::updateRobotState(){
    if(isControlled()){
        info_.nav.highest_target_pos = Eigen::Vector3d(info_.ref.target_x,info_.ref.target_y,0.0);
        if(turnControlled()){
        ...
        if(regions_[Regions::FIRST].inside(info_.nav.highest_target_pos,1.0)){
            info_.nav.highest_target_pos = Eigen::Vector3d(info_.ref.target_x,info_.ref.target_y,10.0);
            robot_state_ = Regions::TOWER;
            debug_tools::Debug().print("OKOK!!!!");
        }
        else if(info_.ref.target_x == 100.0){
            info_.nav.highest_target_pos = Eigen::Vector3d(info_.ref.target_x,info_.ref.target_y,0.0);
        }
        else{
            info_.nav.highest_target_pos = Eigen::Vector3d(info_.ref.target_x,info_.ref.target_y,1.0);
        }
        return;
    }
    else{
